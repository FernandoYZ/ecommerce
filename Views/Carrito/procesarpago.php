...
<script
src="https://www.paypal.com/sdk/js?client-id=ATJ7x54p2QBGDQICnrUDmIroLRmC2kqrZ595A4JGduhE4DokkdaVwmJZ4_INZ3ZxcNhEWF4symfDwR1S&components=buttons">
</script>
							...
							<div class="divmetodpago">
								<div>
									<label for="paypal">
										...
										<img src="<?= media()?>/images/img-paypal.png" alt="Icono de PayPal" class="ml-space-sm" width="auto" height="25">
									</label>
								</div>
								<div>
									<label for="yape">
										<input type="radio" id="yape" class="methodpago" name="payment-method"  value="Yape">
										<img src="<?= media()?>/images/img-yape.png" alt="Icono de Yape" class="ml-space-sm" width="auto" height="25">
								</div>
								<div>
								...
								<div id="divyape" class="notblock" >
									<div>
										<p>Para completar la transacción, te enviaremos a los servidores seguros de Culqi.</p>
									</div>
									<br>
									<button id="btn_pagar" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer">Pagar</button>
								</div>
...
<script>
    ...
    Culqi.publicKey = 'pk_test_3999d292a3cb5972';
    ...
    btn_pagar.addEventListener('click', function (e) {
        monto_total = "<?php echo $total ?>" * 100;
        productos = <?php echo $productosJSON; ?>;
        productos.forEach(producto => {
            arrproduct.push(producto.nombre);
        });
        dir = document.querySelector("#txtDireccion").value;
        ciudad = document.querySelector("#txtCiudad").value;

        Culqi.settings({
            title: 'Glamour Secrets DJR',
            currency: 'PEN',
            amount: monto_total,
        });
    ...
    function culqi() {
        if (Culqi.token) {
            ...
            formData.append('direccion', dir);
            formData.append('ciudad', ciudad);
            formData.append('inttipopago', 6); // Asumiendo que 6 es el ID para Culqi
            ...
            request.onreadystatechange = function () {
                ...
                if (request.status == 200) {
                    try {
                        ...
                        swal("Error", "Hubo un problema al procesar su pedido. Por favor intente de nuevo.", "error");
                    }
                }
            }
        } else {
